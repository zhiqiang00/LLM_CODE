...
class VectorStore:
    ...
    def get_vector(self, EmbeddingModel: BaseEmbeddings) -> List[List[float]]:
        ...
        for doc in tqdm(self.document, desc="Calculating embeddings"):
            self.vectors.append(EmbeddingModel.get_embedding(doc)) # type: ignore
        return self.vectors
    ...
    def query(self, query: str, EmbeddingModel: BaseEmbeddings, k: int = 1, model: Optional[str] = None) -> List[str]:
        query_vector = EmbeddingModel.get_embedding(query, model=model)
